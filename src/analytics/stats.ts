...
export const calculateDailyStats: DailyStatsJob<never, void> = async (
  ...
  try {
    ...
    try {
      ...
      console.warn("Could not fetch total revenue, defaulting to 0:", e?.message);
    }

    let totalViews = 0;
    let prevDayViewsChangePercent = "0";
    try {
      const pageViewData = await getDailyPageViews();
      totalViews = pageViewData.totalViews;
      prevDayViewsChangePercent = pageViewData.prevDayViewsChangePercent;
    } catch (e: any) {
      console.warn("Could not fetch page views (analytics provider not configured?), defaulting to 0:", e?.message);
    }
    ...
    let sources: { source: string; visitors: number }[] = [];
    try {
      sources = await getSources();
    } catch (e: any) {
      console.warn("Could not fetch sources (analytics provider not configured?), skipping:", e?.message);
    }
