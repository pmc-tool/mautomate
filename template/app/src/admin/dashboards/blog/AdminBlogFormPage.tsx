...
export default function AdminBlogFormPage({ user }: { user: AuthUser }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (status === "scheduled" && !publishedAt) {
      toast({ title: "Validation", description: "Scheduled posts need a publish date.", variant: "destructive" });
      return;
    }
    ...
    try {
      ...
      if (isEdit) {
        await updateBlogPost({ id: params.id!, ...payload });
  ...
  return (
    <DefaultLayout user={user}>
      <Breadcrumb pageName={isEdit ? "Edit Blog Post" : "New Blog Post"} />
      ...
      <form className="mx-auto max-w-4xl space-y-6" onSubmit={handleSubmit}>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Write and manage blog post content.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="title">Title *</Label>
              <Input id="title" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="How to improve campaign ROI" required />
            </div>

            <div className="space-y-2">
              <Label htmlFor="slug">Slug</Label>
              <Input id="slug" value={slug} onChange={(e) => setSlug(e.target.value)} placeholder="how-to-improve-campaign-roi" />
            </div>

            <div className="space-y-2">
              <Label htmlFor="excerpt">Excerpt</Label>
              <Textarea id="excerpt" value={excerpt} onChange={(e) => setExcerpt(e.target.value)} placeholder="Short summary shown on listing pages." rows={3} />
            </div>

            <div className="space-y-2">
              <Label htmlFor="content">Content *</Label>
              <Textarea id="content" value={content} onChange={(e) => setContent(e.target.value)} placeholder="Write your post content (markdown or plain text)." rows={16} required />
            </div>
